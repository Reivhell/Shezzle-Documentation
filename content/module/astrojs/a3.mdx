...
description: "Astro components adalah file dengan ekstensi `.astro`. Mereka dirancang sebagai **superset dari HTML**, artinya setiap file HTML valid adalah juga fil..."
category: "astrojs"
...
| `{expression}` | Evaluasi ekspresi JavaScript | `{2 + 2}` → `4` |
...
```astro
---
// ❌ Tanpa Fragment - menambah div yang tidak perlu
const items = ['A', 'B', 'C'];
---
...
### Solusi dengan Fragment:

```astro
---
// ✅ Dengan Fragment - tidak menambah elemen wrapper
const items = ['A', 'B', 'C'];
---
...
**Penting:** Variabel di frontmatter hanya tersedia dalam file tersebut. Tidak otomatis tersedia di layout atau komponen child.
...
**Selanjutnya:** Di Sesi 4, kita akan membahas **Routing & Halaman** - memahami file-based routing, dynamic routes, nested routes, dan API routes. Kita akan melihat bagaimana struktur folder `src/pages/` secara otomatis menjadi URL website kamu.
